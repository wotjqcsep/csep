...
function splitAddressDetail(addr){
  ...
  // extractAddress 의 BLDG 와 같은 목록
  const BW='(?:아파트|빌딩|빌라|오피스텔|타워|상가|맨션|캐슬|파크|프라자|플라자|센터|하이츠|자이|푸르지오|래미안|힐스테이트|하우스|타운|시티|팰리스|스퀘어|빌)';
  // 건물명은 네비가 못 읽는다. 단, 떼고 나서도 앞에 지번·도로명 번호가 남을 때만 뗀다.
  ...
  for(;;){
    ...
    // 목록에 없는 건물명 — 이미 동/호/층을 뗀 뒤여야 하고(그 앞은 건물명일 확률이 높다) 앞에 번지도 남아야 한다
    m=det.length ? a.match(RE_ETC) : null;
    if(m && /[가-힣]/.test(m[1]) && !NOT_BLDG.test(m[1]) && HAS_NUM.test(a.slice(0,m.index))){
      det.unshift(m[1]); a=a.slice(0,m.index).trim(); continue;
    }
    break;
  }
