...
async function openVendorDevices(customerId){
  ...
  const row = cp => {
    ...
    const specs=[['CPU',cp.cpu],['RAM',specSummary('ram',cp.ram)],['SSD',specSummary('ssd',cp.ssd)],['HDD',specSummary('hdd',cp.hdd)],['메인보드',mbSummary(cp.motherboard)],['GPU',cp.gpu],['모니터',cp.monitor],['OS',cp.os],['Office',cp.office_version],['CAD',cp.cad],['Adobe',cp.adobe],['기타1',cp.etc_program1],['기타2',cp.etc_program2],['IP',cp.ip_address],['MAC',cp.mac_address],['프린터',printers],['NAS',cp.nas_name?`${cp.nas_name}${cp.nas_ip?' ('+cp.nas_ip+')':''}${cp.nas_partition_info?' · 파티션 '+cp.nas_partition_info:''}`:''],['공유기',cp.router_name?`${cp.router_name}${cp.router_ip?' ('+cp.router_ip+')':''}${cp.router_hub_count?' · 허브 '+cp.router_hub_count:''}`:''],['시리얼',cp.serial_number],['보증만료',cp.warranty_expiry],['메모',cp.notes]].filter(([,val])=>val);
