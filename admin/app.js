...
function renderEstimates(){ estInit(); const s=estState;
  ...
  <div class="vd-card">
    ...
    <div id="est_url_box" style="display:none;margin-bottom:10px">
      <div style="display:flex;gap:6px"><input id="est_url" placeholder="견적 URL 붙여넣기 (컴퓨존·다나와·아이코다·마이피씨샵 등)" style="flex:1;padding:9px;border:1px solid var(--gray-300);border-radius:8px;font-size:13px">
      ...
      <div style="font-size:11px;color:#0ca678;margin-top:4px">✅ 컴퓨존·다나와·아이코다·마이피씨샵 단일제품·조립PC URL 가져오기 (AI 미사용). <b>가져오기</b>=교체, <b>+ 추가</b>=기존 유지하고 뒤에 추가</div>
      <div style="font-size:11px;color:#e67700;margin-top:3px">💡 아싸컴·조이젠은 URL 차단 → <b>⭐ 북마클릿</b> 사용. 장바구니(로그인 필요)는 모두 북마클릿으로.</div>
    </div>
    ...
    <div id="est_bookmarklet_box" style="display:none;margin-bottom:10px">
      <div style="background:#fffbeb;border:1px solid #fcc419;border-radius:8px;padding:10px">
        <div style="font-size:12px;font-weight:700;color:#e67700;margin-bottom:6px">⭐ 북마클릿 사용법</div>
        <div style="font-size:11px;color:#555;margin-bottom:8px">아래 스크립트를 복사 → 브라우저 즐겨찾기에 새 북마크 추가 → URL 칸에 붙여넣기.<br>컴퓨존·아싸컴·조이젠·다나와·아이코다·마이피씨샵 장바구니·견적·단일상품 페이지에서 클릭하면 자동 전송됩니다.</div>
        <div style="display:flex;gap:6px;align-items:stretch">
          <textarea id="est_bookmarklet_script" readonly style="flex:1;height:60px;padding:8px;border:1px solid var(--gray-300);border-radius:6px;font-size:10px;font-family:monospace;color:var(--gray-600);resize:none">javascript:void((async()=>{const t=document.createElement('div');t.textContent='CSEP: 가져오는 중...';t.style.cssText='position:fixed;top:10px;right:10px;background:#339af0;color:#fff;padding:8px 16px;border-radius:8px;font:bold 14px sans-serif;z-index:999999;transition:opacity .5s';document.body.appendChild(t);const P=new DOMParser();const jR=document.querySelectorAll('tr[class^=cart_list_]');if(jR.length){let i=0;for(const row of jR){i++;t.textContent='CSEP: 사양 '+i+'/'+jR.length;const a=row.querySelector('.cart_name a');if(!a)continue;const h=a.getAttribute('href');if(!h||!h.includes('sInfo'))continue;try{const r=await fetch(h);const b=await r.arrayBuffer();const s=new TextDecoder('euc-kr').decode(b);const doc=P.parseFromString(s,'text/html');const ov=doc.querySelector('.overview');if(ov&&ov.title)row.setAttribute('data-spec',ov.title)}catch(e){}}}const aI=document.querySelectorAll('.list__item.item--acc');if(aI.length){let pids=[];for(const fn of(window.wptg_tagscript_vars||[])){try{const d=typeof fn==='function'?fn():fn;if(d&&d.ty==='Cart'&&d.items)pids=d.items}catch(e){}}for(let i=0;i<aI.length;i++){t.textContent='CSEP: 사양 '+(i+1)+'/'+aI.length;const pid=pids[i]&&pids[i].i;if(!pid)continue;try{const r=await fetch('/shop/product/acc_view.htm?no='+pid);const b=await r.arrayBuffer();const s=new TextDecoder('euc-kr').decode(b);const doc=P.parseFromString(s,'text/html');const el=doc.querySelector('.pro_info__sitename');if(el){let sp=el.textContent.trim();if(sp[0]==='[')sp=sp.slice(1);if(sp[sp.length-1]===']')sp=sp.slice(0,-1);sp=sp.trim();if(sp){const hd=document.createElement('div');hd.style.display='none';hd.setAttribute('data-spec',sp);aI[i].appendChild(hd)}}}catch(e){}}}t.textContent='CSEP: 전송 중...';fetch('https://csep-cf37.onrender.com/api/estimate/import',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({html:document.documentElement.outerHTML})}).then(r=>r.json()).then(d=>{t.textContent=d.ok?'CSEP: '+d.count+'개 ('+location.hostname+')':'CSEP: 실패';setTimeout(()=>{t.style.opacity='0';setTimeout(()=>t.remove(),500)},1500)}).catch(()=>{t.textContent='CSEP: 오류';setTimeout(()=>{t.style.opacity='0';setTimeout(()=>t.remove(),500)},1500)})})())</textarea>
          <button class="btn btn-sm" style="background:#e67700;color:#fff;white-space:nowrap" onclick="const ta=document.getElementById('est_bookmarklet_script');ta.select();navigator.clipboard.writeText(ta.value).then(()=>showToast('스크립트가 복사되었습니다!')).catch(()=>{document.execCommand('copy');showToast('스크립트가 복사되었습니다!')})">📋 복사</button>
        </div>
        <div style="margin-top:6px;font-size:10px;color:var(--gray-400)">지원: 컴퓨존 (장바구니·단일상품·조립PC) · 아싸컴 (장바구니·단일상품·견적) · 조이젠 (장바구니·단일상품·견적) · 다나와 (장바구니·단일상품·조립PC) · 아이코다 (장바구니·단일상품) · 마이피씨샵 (장바구니·단일상품)</div>
      </div>
    </div>
    <div class="table-container"><table class="table">
...
function estAddItems(items, mode){ estSyncAll();
  ...
  (items||[]).filter(it=>!/조립비/i.test(it.cat||'')).forEach(it=>{ const cat=(it.cat||'').trim(), name=(it.name||'').trim(), cost=(Number(it.price)||''), qty=(Number(it.qty)||1), spec=(it.spec||'');
...
function custClean(s){
  ...
  t=t.replace(/\[?\s*미사용\s*탈거\s*제품\s*\]?/gi,'');
  t=t.replace(/단종된\s*모델이므로\s*중고상품\s*주의하시기\s*바랍니다/g,'');
  t=t.replace(/노트북\s*적출상품\s*[/／]\s*[＊*]?\s*병행\s*,?\s*중고\s*판매처\s*확인\s*요망/g,'');
  t=t.replace(/무상\s*\d*\s*년/g,'');
...
async function estUrlImport(btn,mode){
  ...
  const tp=r&&r._totalPrice;
  showToast(items.length+'개 항목을 '+(mode==='append'?'추가':'가져오기')+'했습니다.'+(tp?' (업체 판매가 '+Number(tp).toLocaleString()+'원)':'')+' 매입가·마진 확인 후 인쇄하세요.');
}
...
function payLabel(r){ const pm=PM_LABEL[r.payment_method]||r.payment_method||''; if(r.payment_method==='tax') return '세금계산서'; if(r.tax_invoice) return pm+' (세금계산서)'; return pm; }
...
function renderPayments(){
  ...
  <div class="table-container"><table class="table">
    <thead><tr><th>고객</th><th>결제수단</th><th>입금일</th><th style="text-align:right">공임</th><th style="text-align:right">부품</th><th style="text-align:right">매출</th>${AG?`<th style="text-align:right">${esc(agencyName())} 수수료</th>`:''}<th style="text-align:right;color:#0ca678">환급</th><th style="text-align:right">정산액</th></tr></thead>
    <tbody>${custRows.length? custRows.map(([cid,o])=>{
      ...
      <td style="font-size:12px">${[...o.payments].join(', ')}</td>
