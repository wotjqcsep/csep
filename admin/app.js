...
function stdTaxInvoice(sup,buy,items,sub,vat,total,ctx,memo,copyLabel){
  ...
  return pages.map((pg,pi)=>{
    ...
    const pageNote=pages.length>1?` (${pi+1}/${pages.length})`:'';
    return `<div class="tax-copy" style="width:182mm;height:128mm;box-sizing:border-box;padding:2mm 3mm;overflow:hidden${pi>0?';page-break-before:always':''}">
  ...
  <tr><td colspan="4" style="${lb};font-weight:700">작 성</td><td colspan="13" style="${lb};font-weight:700">공 급 가 액</td><td colspan="10" style="${lb};font-weight:700">세 액</td><td colspan="5" style="${lb}">비 고${pageNote}</td></tr>
  ...
  <tr><td colspan="5" style="text-align:center;font-weight:800;font-size:10px;${b}">${isLast?'₩'+nfmt(total):''}</td><td colspan="5" style="${b}"></td><td colspan="5" style="${b}"></td><td colspan="5" style="${b}"></td><td colspan="5" style="${b}"></td></tr>
  </table>
...
function estPrint(target){
  ...
  const html=`<!doctype html><html><head><meta charset="utf-8"><title>${esc((estState.doctype||'견적서'))} ${no}</title>
    ...
    @media print{ button{display:none} .copy{page-break-inside:avoid} .tax-copy{page-break-inside:avoid} .cut{page-break-after:always} }</style></head><body>
    ${bodyHtml}
