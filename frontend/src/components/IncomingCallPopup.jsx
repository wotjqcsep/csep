...
import { dismissCall, createReception } from '../api'
import './IncomingCallPopup.css'

const STATUS_LABELS = { new: '신규', assigned: '배정', in_progress: '진행중', completed: '완료', cancelled: '취소' }

function IncomingCallPopup({ calls, customers, onDismiss, onRefresh }) {
  const [activeCall, setActiveCall] = useState(null)
  const [form, setForm] = useState({ symptom: '', reception_channel: 'phone', initial_memo: '' })
  const [saving, setSaving] = useState(false)

  const call = activeCall || calls[0]
  if (!call) return null
      ...
      {calls.length > 1 && (
        <div className="call-tabs">
          {calls.map((c, i) => (
            <button
              key={c.id}
              className={`call-tab ${call.id === c.id ? 'active' : ''}`}
      ...
      <div className="call-popup">
        ...
        <div className="call-header">
          ...
          <div className="call-info">
            <div className="call-phone">{formatPhone(call.phone)}</div>
            <div className="call-time">{new Date(call.received_at).toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit', second: '2-digit' })} 수신</div>
          </div>
        ...
        {/* 최근 접수 이력 */}
        {call.recent_receptions?.length > 0 && (
          <div className="call-history">
            <div className="call-history-title">최근 접수 이력</div>
            {call.recent_receptions.map((r, i) => (
        ...
        {customer && (
          <div className="call-form">
            <div className="call-form-row">
              <select value={form.reception_channel} onChange={set('reception_channel')} className="call-select">
